...
| 5   | When James evaluates service quality across vehicle categories, he wants to compare average driver ratings by vehicle type so he can monitor customer satisfaction trends. | ✅ Implemented  | Clicking a bar filters all other charts by that vehicle type.|
...
| `qc_line_chart`   |	Output	| `@render_plotly`      |   `QueryChat`           | #4 |
...
filters rows to the selected date range. The filtered DataFrame is then used to create a pie chart showcasing split of revenue across vehicle types with the only outputs consuming it 
being `pie_chart` and `rating_bar`.
